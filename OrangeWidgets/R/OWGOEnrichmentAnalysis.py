"""
<name>RGO Enrichment Analysis</name>
...
class OWGOEnrichmentAnalysis(OWRpy):
    ...
    def __init__(self, parent=None, signalManager=None, name="GO Enrichment Analysis"):
        OWRpy.__init__(self, parent, signalManager, name)
        self.inputs = [("Cluster Examples", RvarClasses.RDataFrame, self.SetClusterDataset, Default), ("Reference Examples", RvarClasses.RDataFrame, self.SetReferenceDataset, Single + NonDefault)] #, ("Structured Data", DataFiles, self.chipdata, Single + NonDefault)]
        self.outputs = [("Selected Examples", RvarClasses.RDataFrame, Default), ("Unselected Examples", RvarClasses.RDataFrame, Default), ("Example With Unknown Genes", RvarClasses.RDataFrame, Default)] #, ("Selected Structured Data", DataFiles, Single + NonDefault)]

        self.setRvariableNames(['selected','unselected'])
            ...
            for ex in self.clusterDataset:
                if not ex[geneAttr].isSpecial() and any(gene in selectedGenes for gene in str(ex[geneAttr]).split(",")):
                    ...
                    for gene in str(ex[geneAttr]).split(","):
                        selectedExamples.append(gene)
                else:
                    for gene in str(ex[geneAttr]).split(","):
                        unselectedExamples.append(gene)
                        
            self.R(self.Rvariables['selected'] + '=data.frame(id=c("'+ '","'.join(selectedExamples) + '"))')
            self.R(self.Rvariables['unselected'] + '=data.frame(id=c("'+ '","'.join(unselectedExamples) + '"))')
            self.rSend("Selected Examples",{'data':self.Rvariables['selected']})
            self.rSend("Unselected Examples",{'data':self.Rvariables['selected']})
