...
def Rcommand(query, callType = 'getRData', processingNotice=False, silent = False, showException=True, wantType = None, listOfLists = True):
    rst = RSessionThread()
    ...
    try:
        if callType == 'getRData':
            ...
            rst.run('tmp<-('+query+')')
            output = rst.run('list(rowNames=rownames(tmp), colNames=colnames(tmp), Length=length(tmp), Class=class(tmp), Summary=summary(tmp))')
            rst.run('rm(tmp)')
        else:
            rst.run(query) # run the query anyway even if the user put un a wierd value
