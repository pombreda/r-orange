...
class ED(OWRpy): 
    ...
    def __init__(self, parent=None, signalManager=None):
        OWRpy.__init__(self, parent, signalManager, "Effective Dose", wantMainArea = 0, resizingEnabled = 1)
        self.setRvariableNames(["ED"])
        self.data = {}
        self.loadSettings() 
        self.RFunctionParam_object = ''
        self.inputs = [("object", signals.RModelFit, self.processobject)]
        self.outputs = [("ED Output", signals.RVariable)]
        
        self.help.setHtml('<small>Calculates the effective dose for a dose response object.</small>')
