...
def removeWidgetIcon(icon):
    global _widgetIcons
    for t in _widgetIcons.values():
        log.log(10, 5, 3, 'widget icon values %s' % str(t))
        while icon in t:
            log.log(10, 5, 3, 'removing widget icon instance %s' % icon)
            t.remove(icon)
###########################
######  instances       ###
###########################
...
def instances(wantType = 'list'):
    ...
    if wantType == 'list':## return all of the instances in a list
        log.log(10, 5, 3, 'Widget instances are %s' % unicode(_widgetInstances.values()))
