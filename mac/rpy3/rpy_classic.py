    ...
    if hasattr(obj, '__len__'):
        if obj.typeof in [ri.INTSXP, ri.REALSXP, ri.CPLXSXP,
            ...
            try:
                ...
                res = dict([(obj_names[i], rpy3py(x)) for i,x in enumerate(obj)])
            except LookupError:
                res = [rpy3py(x) for x in obj]
...
class Robj(object):
    ...
    def as_py(self, mode = None):
        if mode is None:
            mode = default_mode
        res = rpy3py(self.__sexp, mode = mode)
        return res
